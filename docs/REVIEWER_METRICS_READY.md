...
### 1.1 Endpoint and parameter inventory (discovered corpus)

| Metric | Value |
|---|---:|
| Total endpoints discovered | 60 |
| Endpoints with parameters | 52 |
| Total endpoint-parameter pairs | 50 |
...
- `Total endpoint-parameter pairs`: unique `(endpoint path, parameter)` combinations.
...
| XXE | 2 | 0 | 0 | 1.00 | 1.00 | 1.00 |

### 5.2 Detection speed (hybrid)

Computed as `elapsed_seconds / TP` from benchmark + scoped evaluation report.
...
- This report uses scoped-path evaluation for detection tables (`evaluation/output/evaluation_report_scoped.json`).
- Keep one protocol consistent per table in the paper and state it explicitly.
